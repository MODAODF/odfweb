...
class LocalRootScanner extends Scanner {
	...
	private function shouldScanPath(string $path): bool {
		$path = trim($path, '/');
		$res = $path === '' || strpos($path, 'appdata_') === 0 || strpos($path, '__groupfolders') === 0 || strpos($path, '__grouprepos') === 0 || strpos($path, '__templaterepo') === 0 || strpos($path, '__mergeodf') === 0 || strpos($path, 'updaterTmp-') === 0;
		return $res;
	}
}
